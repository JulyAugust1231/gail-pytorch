...
class SafeSarsaAgent(object):
    ...
    def __init__(self,
        ...
        if self.args.env_name == "Gridworld":
            ...
            self.review_model = ValueNetwork(self.state_dim[0]).to(self.device)  # reward

            self.target_cost_model = OneHotDQN(self.state_dim[0], self.action_dim).to(self.device)
            self.target_review_model = ValueNetwork(self.state_dim[0]).to(self.device)
